...
SliceFlash.py $1 -o oflash_composition_dens.eps -rd=0.0 -ru=1.5e4 -zd=-2.0e4 -zu=1.0e4 --densitycontour=$2

# Nice demonstration of density contour. Plot density with a density contour.
SliceFlash.py $1 -o oflash_dens_contour.eps -rd=0.0 -ru=1.5e4 -zd=-2.0e4 -zu=1.0e4 -field='dens' -cmap='spring' --densitycontour=$2 --densitycontourwidth=0.5
