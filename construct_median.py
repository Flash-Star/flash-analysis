...
#for h in headers:
#	if h!='time':
#		median[h] = [0.5*(max([dint[h](t_interp)])+min([dint[h](t_interp)])) for k,dint in data_interps.iteritems() for t_interp in t_interp_vec]

median['time'] = t_interp_vec
		...
		if h!='time':
			...
			for k,dint in data_interps.iteritems():
				try:
					...
					continue
			median[h].append(0.5*(max(h_vec)+min(h_vec)))

for h in headers:
	median[h] = np.array(median[h])

print 'Finished constructing median dataset!'

# Write the median out to a file
ofile = open(pref+'median.dat','w')
ofile.write('#')
for h in headers:
	ofile.write(h + '  ')
ofile.write('\n')
for n in xrange(0,len(t_interp_vec)):
	for h in headers:
		if h=='time':
			...
			ofile.write('{:1.14e}'.format(median[h][n]) + '  ')
	ofile.write('\n')
ofile.close()
